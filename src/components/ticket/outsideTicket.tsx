...
const OutsideTicket = ({flightClass, price, dst, src, logoStyle, logoSrc, setOpen}: OutsideTicketDetailsProps) => {
    return (
        <div
            className="w-[60%] h-[200px] rounded-xl flex justify-center items-center text-center select-none mt-4 absolute -bottom-[200px] overflow-hidden bg-white flip-box-back cursor-pointer"
            ...
            <div
                className={"absolute bottom-0 left-1/2 border-2 border-dashed border-[#555] rounded px-10 font-bold xl:text-3xl sm:text-lg"}
            ...
            <div className={"w-[90%] flex justify-between items-center"}>
                <div className={"w-[55%] flex justify-between"}>
                    <div>
                        <div className={"text-gray-500 sm:text-lg"}> {Truncate(dst.country, 15)}</div>
                        <div className={"font-bold xl:text-3xl sm:text-lg"}>{FormattedTime(dst.time)}</div>
                        <div className={"text-gray-500 sm:text-lg"}>{FormattedMonthDate(dst.time)}</div>
                    </div>
